...
const TrendsScreen = () => {
  ...
  const renderInsight = () => {
    const saunaSeries = extractSeries('saunaDuration');
    const total = saunaSeries.reduce((a, b) => a + b, 0);
    const avg = (total / saunaSeries.length).toFixed(1);
    const max = Math.max(...saunaSeries);

    return `🔥 Sauna — Avg: ${avg} min | Max: ${max} min | Total: ${total} min`;
  };
  ...
  return (
    ...
    >
      ...
      <Text style={[styles.insightText, { color: theme.colors.text }]}>🧠 Insight: You're building a great routine! Keep it going.</Text>

      <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>🧊 Cold Plunge Duration (min) ℹ️</Text>
      <BarChart
      ...
      <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>🌡️ Cold Plunge Temp (°F) ℹ️</Text>
      <LineChart
      ...
      <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>🔴 Red Light Duration (min) ℹ️</Text>
      <BarChart
      ...
      <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>🌍 Grounding Duration (min) ℹ️</Text>
      <LineChart
      ...
      <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>🔥 Sauna Duration (min) ℹ️</Text>
      <BarChart
      ...
      <Text style={[styles.sectionTitle, { color: theme.colors.text, marginTop: 20 }]}>🔍 Summary</Text>
      <Text style={[styles.insightText, { color: theme.colors.text }]}>{renderInsight()}</Text>
    </ScrollView>
  );
};
